...
import sys, os, subprocess, pickle, tifffile
import numpy as np
from datetime import datetime
import matplotlib.pyplot as plt
import py3DXRD
from py3DXRD.p212_tools import load_p212_log, load_p212_fio, parse_p212_command
from py3DXRD.angles_and_ranges import convert_ranges, mod_360
...
class DataAnalysis:
    ...
    def process_images(self, frames = None, save_tifs = False, q0_pos = None, rad_ranges = None, thr = None):
        for SP in self.sweepProcessors:
            print(single_separator + f'\nPROCESSING for {SP.name}:')
            if not os.path.exists(SP.directory): os.makedirs(SP.directory) # Create the directory if does not exist.

            ### Loading and processing images:
            if type(frames) == list: SP.chunk['frames'] = frames # If substack is needed.
            SP.load_sweep()  # Loading and processing images usually takes SOME time.
            if save_tifs: SP.save_tifs() # Usually not needed.
            SP.process_imgs(bckg = 'auto50') # bckg = 'auto50' - median over equally spaced images (not more than ~50).
            
            SP.calculate_projections(q0_pos = q0_pos, rad_ranges = rad_ranges) # Usually takes LONG time.
            SP.plot()
    ...
    def peaksearch(self, peaksearch_thresholds, peakmerge_thresholds, min_peak_dist, use_temp_tifs = False, del_temp_tifs = False):
        for SP in self.sweepProcessors:
            ...
            yaml_file = SP.save_peaksearch_yaml(thresholds = peaksearch_thresholds, spline_path = SP.geometry.spline_file)
            SP.run_peaksearch(yaml_file = yaml_file, use_imgs = False, use_temp_tifs = use_temp_tifs, del_temp_tifs = del_temp_tifs)  # Usually takes LONG time.
            for thr in peaksearch_thresholds:
                ...
                PI.save_flt(flt_file = SP.name + f'_peaks_t{thr}_backup.flt', overwrite = True)
                ...
                PI.save_flt(flt_file = SP.name + f'_peaks_t{thr}.flt', overwrite = True)
                if len(PI.peaks)<1 and thr in peakmerge_thresholds:
                    peakmerge_thresholds.remove(thr)
                    
            yaml_file = SP.save_peaksearch_yaml(thresholds =  peakmerge_thresholds, pix_tol = min_peak_dist)
            SP.merge_peaks(yaml_file = yaml_file)
            SP.set_attr('imgs', None) # As images are too heavy and not needed futhermore.
            pickle.dump(SP, open(SP.directory + SP.name + "_SweepProcessor.p", "wb") )
    ...
    def index(self, move_det_xyz_mm = [0, 0, 0]):
        if not move_det_xyz_mm: move_det_xyz_mm = [0, 0, 0]
        self.set_attr('peakIndexers', [])
        for iP, SP in enumerate(self.sweepProcessors):    # (e.g. Varex 1,2,3,4).
            print(single_separator + f'\nINDEXING of {SP.name}:')
            ### Evaluating peaks:                
            PI = SP.generate_PeakIndexer()
            if move_det_xyz_mm != [0,0,0]:
                PI.geometry.move_detector(move_det_xyz_mm)
            PI.run_indexer(gve_file = PI.name + '_' + self.material['name'] + '.gve')
    ...
    def evaluateGvectors(self, tth_gap=0.5, ds_gap=0.1, eta_gap=1, omega_gap = None, to_plot = True, save_arrays = False):
        ...
        self.gvectorEvaluator.remove_not_inrings()
        if not omega_gap: omega_gap = 2*abs(self.sweepProcessors[0].sweep['omega_step'])
...
#             GE.group_gvectors(0.1, 1, 1)
    ...
    def searchGrains(self, grainSpotter = None):
        ...
        if grainSpotter: self.set_attr('grainSpotter', grainSpotter)
        ...
        self.grainSpotter.set_attr('domega', abs(self.sweepProcessors[0].sweep['omega_step']))
        
        self.grainSpotter.run_grainspotter(gve_file = self.name+'.gve', log_file = self.name+'.log')  # Usually takes SOME time.
        self.set_attr('grains', py3DXRD.Grain.load_log(directory=self.directory, log_file=self.grainSpotter.log_file))
        pickle.dump(self.grainSpotter, open(self.directory+self.name+"_GrainSpotter.p","wb") )
        for g in self.grains: g.identify_measured(self.gvectorEvaluator.gvectors)
        
    
    def runPolyXSim(self, polyxsim=None):
        print(single_separator + f'\nRUNNING POLYXSIM for {self.name}')
        sim_dir = self.directory+'sim_'+self.material['name']+'/'
        if not os.path.exists(sim_dir): os.makedirs(sim_dir) # Create the directory if does not exist
        ...
        for g in self.grains:
            print(single_separator+f'\nGRAIN {g.grain_id}')
            if not g.size: g.set_attr('size', 0.05)
#             g.identify_measured(self.gvectorEvaluator.gvectors)
            ...
            for SP in self.sweepProcessors:
                ...
                PX.set_attr('inp_file' , SP.name+'.inp')
                PX.set_attr('geometry' , SP.geometry)
                PX.set_attr('grains', [g])
                PX.save_inp(directory=sim_dir, inp_file=f'sim_g{g.grain_id:03d}_d{det_num}.inp', overwrite = True)
                d = min(0.2, np.sqrt( g.position[0]**2 + g.position[1]**2 + g.position[2]**2) )
                PX.set_attr('sample_cyl', [2*d, 2*d])
                polyxsim.run_PolyXsim()  # Usually takes SOME time.
                PX.run_PolyXsim()  # Usually takes SOME time.
                GE_this_det = py3DXRD.GvectorEvaluator(directory = PX.direc)
                GE_this_det.set_attr('geometries', [PX.geometry])
                GE_this_det.load_gve(gve_file = PX.stem+'.gve')
                try:
            ...
            ind = np.argsort([g['ds'] for g in GE_sim.gvectors]) # Sort in asceding ds
    ...
    def mark_peaks(self):
        ...
        gvectors = []
        for G in self.grains: gvectors += G.measured_gvectors
        ...
        for g in gvectors:
            ...
            spot3d_id = g['spot3d_id'] - self.gvectorEvaluator.spot3d_id_reg*GE_id
            if GE_id == 0:
            ...
            if len(p_list) > 0:
                ...
                raise ValueError('peak for gvector '+str(g['spot3d_id']) +' not found!')
...
#                 eta_ind = round(g['eta']-180)
            ...
            SP_FULL.add_to_log('Writing file: '+path+"_immax_rngs_marked.tif", True)
            SP.add_to_log(     'Writing file: '+path+"_immax_rngs_marked.tif", True)
            tifffile.imsave(path+"_immax_rngs_marked.tif", immax_rngs)
            for ir, rng in enumerate(ranges):
                SP_FULL.add_to_log('Writing file: '+path+"_omgeta_rngs_marked.tif", True)
                SP.add_to_log(     'Writing file: '+path+"_omgeta_rngs_marked.tif", True)
                tifffile.imsave(path+f"_omgeta_rngs_marked-{rng[0]}-{rng[1]}.tif", omgeta_rngs[ir])
                
            pickle.dump(SP_FULL, open(SP.directory + SP.name + "_SweepProcessor.p", "wb") )
...
def plot_sinogram(list_DATApaths):
    import tifffile
    omegas = []
    ypos_s = []
    for path in list_DATApaths:
        ...
        for SP in DATA.sweepProcessors:
            PI = SP.generate_PeakIndexer()
            PI.geometry.move_detector([0, -1*DATA.position[1], 0])
    ...
    omgy = np.zeros([len(SP.chunk['omegas'])+2, len(list_DATApaths)+2], dtype=np.float32)
    omg_min = min(omegas)
    ...
    tifffile.imsave(DATA.directory.replace('/' + DATA.directory.split('/')[-2],'') + 'sinogram.tif', omgy)
    
def set_MultiDATA(list_DATApaths, num_0):
    def new_dir(old_dir):
        return old_dir.replace(old_dir.split('/')[-2]+'/', '')

    def new_name(old_name):
        return old_name.replace(f'_f{i_fast:03d}_', '_all_')

    path = list_DATApaths[num_0]
    print("PATH:", path)
    DATA = pickle.load( open(path,"rb") )
    DATA.directory = new_dir( DATA.directory)
    DATA.name      = new_name(DATA.name)

    for ii, SP in enumerate(DATA.sweepProcessors):
        DATA.sweepProcessors[ii].directory = new_dir( DATA.sweepProcessors[ii].directory )
        DATA.sweepProcessors[ii].name      = new_name(DATA.sweepProcessors[ii].name)
        DATA.sweepProcessors[ii].geometry.directory = new_dir( DATA.sweepProcessors[ii].geometry.directory )
        DATA.sweepProcessors[ii].geometry.par_file  = new_name( DATA.sweepProcessors[ii].geometry.par_file )

    for ii, PI in enumerate(DATA.peakIndexers):
        DATA.peakIndexers[ii].directory = new_dir( DATA.peakIndexers[ii].directory )
        DATA.peakIndexers[ii].name = new_name(DATA.peakIndexers[ii].name)
        DATA.peakIndexers[ii].geometry.directory = new_dir( DATA.peakIndexers[ii].geometry.directory )
        DATA.peakIndexers[ii].geometry.par_file  = new_name( DATA.peakIndexers[ii].geometry.par_file )

    DATA.gvectorEvaluator.directory = new_dir( DATA.gvectorEvaluator.directory )
    DATA.gvectorEvaluator.name      = new_name(DATA.gvectorEvaluator.name)
    for ii, GM in enumerate(DATA.gvectorEvaluator.geometries):
        DATA.gvectorEvaluator.geometries[ii].directory = new_dir( DATA.gvectorEvaluator.geometries[ii].directory )
        DATA.gvectorEvaluator.geometries[ii].par_file  = new_name(DATA.gvectorEvaluator.geometries[ii].par_file)

    for path in list_DATApaths:
        if path == list_DATApaths[num_0]: continue
        DATA_ = pickle.load( open(path,"rb") )
        DATA_.index(move_det_xyz_mm = list(np.asarray(DATA.position) - np.asarray(DATA_.position)))
        DATA_.evaluateGvectors(tth_gap=0.5, ds_gap=0.1, eta_gap=1)
