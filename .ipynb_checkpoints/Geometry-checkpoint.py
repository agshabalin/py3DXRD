...
class Geometry:
    ...
    def move_detector(self, shift_xyz_in_mm): # in FABLE coordinates
        self.add_to_log(f'Moving detector by: {shift_xyz_in_mm} (XYZ in mm)', True)
    ...
    def save_yml(self, directory = None, yml_file = None, overwrite = False):
        if directory: self.set_attr('directory', directory)
        if yml_file: self.set_attr('yml_file', yml_file)
        if not self.yml_file: self.set_attr('yml_file', self.par_file.replace('.par', '.yml'))
        if not os.path.exists(self.directory):
            os.makedirs(self.directory)
            self.add_to_log('Created directory: '+self.directory, True)
    ...
    def average_geometries(list_of_geometries):
        ...
        GM.wavelength = None if str(a)=='nan' else a
        
        a = nanmean([g.unitcell[0] for g in L if g.unitcell[0]!=None])
        GM.unitcell[0] = None if str(a)=='nan' else a
        
        a = nanmean([g.unitcell[1] for g in L if g.unitcell[1]!=None])
        GM.unitcell[1] = None if str(a)=='nan' else a
        
        a = nanmean([g.unitcell[2] for g in L if g.unitcell[2]!=None])
        GM.unitcell[2] = None if str(a)=='nan' else a
        
        a = nanmean([g.distance for g in L if g.distance!=None])
        GM.distance = None if str(a)=='nan' else a

        a = nanmean([g.tilt[0] for g in L if g.tilt[0]!=None])
        GM.tilt[0] = None if str(a)=='nan' else a

        a = nanmean([g.tilt[1] for g in L if g.tilt[1]!=None])
        GM.tilt[1] = None if str(a)=='nan' else a

        a = nanmean([g.tilt[2] for g in L if g.tilt[2]!=None])
        GM.tilt[2] = None if str(a)=='nan' else a

        a = nanmean([g.t[0] for g in L if g.t[0]!=None])
        GM.t[0] = None if str(a)=='nan' else a

        a = nanmean([g.t[1] for g in L if g.t[1]!=None])
        GM.t[1] = None if str(a)=='nan' else a

        a = nanmean([g.t[2] for g in L if g.t[2]!=None])
        GM.t[2] = None if str(a)=='nan' else a
        
        a = nanmean([g.y_center for g in L if g.y_center!=None])
        GM.y_center = None if str(a)=='nan' else a

        a = nanmean([g.z_center for g in L if g.z_center!=None])
        GM.z_center = None if str(a)=='nan' else a
    ...
    def save_geometries_as_yml(list_of_geometries, directory, yml_file, overwrite = False):
        ...
        pars = GM.into_hexrd_definitions(det_num = 1)
